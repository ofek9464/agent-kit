...
Map the plan as a decision tree. A question belongs on the current frontier only when its prerequisites are already settled.

Ask the whole frontier in one round. Number the questions, explain the decision each one controls, and give a recommended answer with a short reason. Put dependent questions in later rounds.
...
Completion means the frontier is empty, material assumptions are visible, and the user confirms the shared understanding.
