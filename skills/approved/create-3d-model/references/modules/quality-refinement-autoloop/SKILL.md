...
4. update the package `SKILL.md`, `manifest.json`, provenance, UI metadata, and release notes in the owning repository when relevant;
...
- `scripts/release_readiness_check.py` performs lightweight Pi-package manifest, module-path, metadata, license, and connector checks before commit/push; it also retains compatibility with the original upstream repository layout.
