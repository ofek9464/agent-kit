...
allowed-tools: Read Bash Glob Grep execute_blender_code get_scene_info get_object_info get_viewport_screenshot
---
...
3. **Execute** code in Blender via `execute_blender_code`, converting curves to meshes with PBR materials.
4. **Export** as optimized GLB (≤ 15 MB), validating size and topology.

You (Codex) are the orchestrator. The `scripts/` directory contains the only standalone code (`wireframe_analyzer.py`); everything else is patterns you emit and run via MCP.

## Prerequisites — check first

Before any wireframe work, verify the environment:

1. **Blender MCP is reachable**. Call `get_scene_info`. If it errors with "Could not connect to Blender", stop and tell the user:
...
```bash
python3 /absolute/path/to/create-3d-model/references/modules/wireframe-to-3d/scripts/wireframe_analyzer.py \
  /absolute/workspace/path/input.png \
  /absolute/workspace/path/output.json
```
...
1. `get_scene_info` — confirm expected objects exist.
2. For paired parts (left/right lens), call `get_object_info` on each and compare bounding box widths. Tolerance: 1 mm.
...
Treat wireframe extraction as the structural first pass, not the presentation result. Validate the current task with local before/after renders: the trace should establish the outline, while the chained modeling, material, lighting, and camera passes should produce the finished asset.

**The orchestrator (`create-3d-model`) should always plan for the chain** when the user asks for a "model of X" from a wireframe.
