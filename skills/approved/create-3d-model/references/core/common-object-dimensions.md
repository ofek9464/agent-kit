...
**This is the kind of subject the skill CANNOT do well automatically.** Production-quality character work requires sculpting, retopology, authored textures, and manually shaped facial or deformation targets. Treat primitive recipes as blockouts, not finished character work.
...
**Honest scope statement**: warn the user that automated primitive generation produces a recognizable silhouette only. For production-quality character output, use an authorized base mesh and sculpt/retopology workflow or recommend a Blender character artist.
...
1. **Import an existing human base mesh** via the Blender MCP's other tools:
   - `download_polyhaven_asset` — Poly Haven has CC0 character assets
   - `download_sketchfab_model` — Sketchfab CC-BY models
   - `generate_hyper3d_model_via_text` — text-to-3D AI generation
...
3. **Use a Blender character artist** — provide the references, target topology, rigging needs, texture resolution, facial-expression requirements, and export target as a concrete brief.
...
The `create-3d-model` orchestrator should read this file (at most once per session) before generating any modeling code. Pattern:
