...
Diagnosis alone does not authorize a fix. If the user asked only to diagnose, stop after reporting the cause and a scoped repair option. If the user asked to fix it, make the smallest root-cause change, verify it against the reported failure, and add a regression test when a meaningful seam exists.
